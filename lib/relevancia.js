...
export function calcularRelevancia(ayuda, perfil) {
  ...
  // Fallback: detectar nominativas por patrón de nombre aunque el enricher no las marque
  const _n = ayuda.nombre || ''
  if (/^subvenci[oó]n directa (al?|a la|als?) .{3,60}$/i.test(_n) &&
      !/cobratelo|ciudadano|persona|familia|menor|autonomo|empresa/i.test(_n)) return 0
  if (/subvenci[oó]n.*asociaci[oó]n (de |del |dels )/i.test(_n)) return 0
  ...
  if (esPensionista) {
    ...
    if (/solicitar.*jubilac|tramitar.*jubilac|acceder.*jubilac|como.*jubilarse|pedir.*jubila|acces.*jubilaci|solicitud.*pension.*jubila|dar.*(se|se).*alta.*jubil|pension.*jubilacion contributiva|pensio.*jubilacio contributiva/.test(t)) return 0
    // Excluir: orfandad (es para hijos de fallecidos, no para el viudo/a)
    if (/orfand|orfan/i.test(t)) return 0
  ...
  // VIUDO/A sin hijos a cargo — excluir orfandad y prestaciones para hijos de fallecidos
  if (familia.includes('viudo') && !tieneHijos) {
    if (/orfand|orfan|fill.*orfe|hijo.*orfe|menor.*orfe/i.test(t)) return 0
  }
