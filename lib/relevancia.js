...
export function calcularRelevancia(ayuda, perfil) {
  ...
  // Principio: EXCLUIR salvo que confirmemos que es de la zona del usuario.
  // Prioridad del dato geográfico: entidades_geo (enricher) > comunidad_autonoma > texto.
  // ════════════════════════════════════════════════════════════

  // Filtro por entidades_geo (municipios/provincias concretas extraídas por el enricher).
  // Es el dato más fiable: si la ayuda está atada a lugares concretos, deben coincidir
  // con el municipio, comarca o provincia del usuario. Si no, fuera.
  if (Array.isArray(ayuda.entidades_geo) && ayuda.entidades_geo.length > 0) {
    const zonasUsuario = [pueblo, comarca, provincia, ccaa].filter(Boolean).map(norm)
    const geoAyuda = ayuda.entidades_geo.map(norm)
    // Coincide si alguna entidad de la ayuda encaja con alguna zona del usuario
    const coincide = geoAyuda.some(g => zonasUsuario.some(z => z && (g.includes(z) || z.includes(g))))
    if (!coincide) return 0
  }

  if (ayuda.ambito === 'autonomico') {
    if (!ccaa) return 0
    // Si la ayuda declara CCAA y es distinta → fuera (caso claro).
    if (ayuda.comunidad_autonoma && norm(ayuda.comunidad_autonoma) !== 'estatal' && norm(ayuda.comunidad_autonoma) !== norm(ccaa)) return 0
    // Si NO declara CCAA (autonómica sin zona confirmada), no la damos por válida a ciegas:
    // solo pasa si su texto u organismo mencionan la CCAA/provincia del usuario.
    // Esto evita el caso "ayuda autonómica de otra región con CCAA vacía" (bug Madrid).
    if (!ayuda.comunidad_autonoma || norm(ayuda.comunidad_autonoma) === 'estatal') {
      const zonasU = [ccaa, provincia, comarca, pueblo].filter(Boolean).map(norm)
      const orgNorm = norm(ayuda.organismo || '')
      const mencionaZona = zonasU.some(z => z && (t.includes(z) || orgNorm.includes(z)))
      if (!mencionaZona) return 0
    }
  }
  ...
  if (ayuda.renta_max) {
    ...
    // Si el usuario NO indicó ingresos y la ayuda tiene un tope de renta BAJO
    // (típico de ayudas de vulnerabilidad), no la mostramos: no podemos confirmar
    // que encaje, y mostrar una ayuda de renta baja a quien no consta que la tenga
    // es un falso positivo que resta credibilidad.
    if (rentaUsuario === 0 && ayuda.renta_max <= 12000) return 0
  }
  ...
  if (ayuda.edad_min || ayuda.edad_max) {
    ...
    if (nac) {
      ...
      // El usuario no indicó edad. Si la ayuda tiene un rango de edad ESTRECHO
      // (p.ej. becas juveniles 18-30, ayudas a mayores 65+), no la mostramos:
      // es muy probable que no encaje y es de las que más chirrían al usuario.
      const min = ayuda.edad_min || 0
      const max = ayuda.edad_max || 120
      const rangoEstrecho = (max - min) <= 20 || min >= 60 || max <= 35
      if (rangoEstrecho) return 0
    }
  }
