...
export function calcularRelevancia(ayuda, perfil) {
  ...
  if (provincia) {
    ...
    if (excluir.some(p => t.includes(p))) {
      if (t.includes('giron')) console.log('[RELEV] excluido por provincia:', ayuda.nombre?.substring(0,40), 'prov:', provincia, 'norm:', norm(provincia))
      return 0
    }
    if (t.includes('giron')) console.log('[RELEV] Girones PASA filtro prov:', provincia, 'norm:', norm(provincia), 'excluir_len:', excluir.length)
  }
  ...
  if (ayuda.es_nominativa === true) return 0
