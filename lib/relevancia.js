...
export function calcularRelevancia(ayuda, perfil) {
  ...
  if (['municipal','comarcal'].includes(ayuda.ambito)) {
    ...
    // La ayuda municipal debe ser de la misma CCAA que el usuario
    if (ayuda.comunidad_autonoma && ccaa &&
        norm(ayuda.comunidad_autonoma) !== norm(ccaa)) return 0
  }

  // Las ayudas autonómicas también deben coincidir en CCAA (ya cubierto arriba)
  // Para municipales sin CCAA explícita: filtrar por exclusión textual de otras provincias
