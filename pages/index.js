        ...
        <section style={{ background: '#fff', width: '100%', padding: '96px 0' }}>
          <div style={{ maxWidth: 1024, margin: '0 auto', padding: '0 24px' }}>
            ...
            <h2 className="font-display font-bold" style={{ fontSize: 'clamp(26px,3.5vw,40px)', color: '#1a0d00', letterSpacing: '-1px', marginBottom: 8, textAlign: 'center' }}>
              Ayudas reales. Importes reales. Enlace directo.
            </h2>
            <p style={{ color: '#7a4a1a', fontSize: 15, textAlign: 'center', marginBottom: 48, maxWidth: 520, margin: '0 auto 48px' }}>
              En 2 minutos ves exactamente qué puedes cobrar, con los requisitos y el enlace oficial de cada convocatoria.
            </p>

            {/* Mockup del panel de resultados */}
            <div style={{ background: '#fff', border: '1px solid #F5C89A', borderRadius: 20, overflow: 'hidden', boxShadow: '0 20px 60px rgba(50,26,0,0.12)' }}>
              {/* Header del panel */}
              <div style={{ background: '#321A00', padding: '16px 24px', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                <div>
                  <p style={{ color: 'rgba(255,245,235,0.5)', fontSize: 11, letterSpacing: '1px', marginBottom: 2 }}>AUTÓNOMO · BARCELONA · 38 AÑOS</p>
                  <p style={{ color: '#FFF5EB', fontSize: 16, fontWeight: 700 }}>14 ayudas disponibles para ti</p>
                </div>
                <div style={{ background: '#FF8300', color: '#321A00', fontSize: 12, fontWeight: 800, padding: '6px 14px', borderRadius: 100 }}>
                  12.400€ detectados
                </div>
              </div>
              ...
              {/* Lista de ayudas */}
              {[
                { org: 'Red.es · Gobierno de España', nombre: 'Kit Digital — Presencia web y SEO', importe: '2.000€', estado: 'Disponible', color: '#16a34a' },
                { org: 'SEPE', nombre: 'Cuota cero autónomos primer año', importe: '1.200€', estado: 'Disponible', color: '#16a34a' },
                { org: 'Ministerio de Vivienda', nombre: 'Bono Alquiler Joven 2026', importe: '2.400€/año', estado: 'Disponible', color: '#16a34a' },
                { org: 'ACCIÓ · Generalitat de Catalunya', nombre: 'Cupons per a la digitalització', importe: '3.000€', estado: 'Disponible', color: '#16a34a' },
                { org: 'ICEX España', nombre: 'Programa XPANDE — Internacionalización', importe: '1.800€', estado: 'Próximamente', color: '#FF8300' },
                { org: 'Generalitat de Catalunya', nombre: 'Ajuts per a la formació professional', importe: '2.000€', estado: 'Disponible', color: '#16a34a' },
              ].map((a, i) => (
                <div key={i} style={{
                  padding: '16px 24px',
                  borderBottom: i < 5 ? '1px solid #FFF0E0' : 'none',
                  display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 16,
                  background: i % 2 === 0 ? '#fff' : '#FFFAF5',
                }}>
                  <div style={{ flex: 1 }}>
                    <p style={{ fontSize: 11, color: '#7a4a1a', marginBottom: 3 }}>{a.org}</p>
                    <p style={{ fontSize: 14, fontWeight: 600, color: '#1a0d00' }}>{a.nombre}</p>
                  </div>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 12, flexShrink: 0 }}>
                    <span style={{ fontSize: 15, fontWeight: 700, color: '#FF8300' }}>{a.importe}</span>
                    <span style={{ fontSize: 11, fontWeight: 600, color: a.color, background: a.color + '15', padding: '3px 10px', borderRadius: 100 }}>{a.estado}</span>
                    <span style={{ fontSize: 12, color: '#FF8300', cursor: 'pointer' }}>Ver →</span>
                  </div>
                </div>
              ))}

              {/* Footer del panel */}
              <div style={{ padding: '16px 24px', background: '#FFFAF5', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                <p style={{ fontSize: 12, color: '#7a4a1a' }}>Mostrando 6 de 14 ayudas detectadas</p>
                <Link href="/login" style={{ background: '#FF8300', color: '#fff', padding: '10px 20px', borderRadius: 8, fontSize: 13, fontWeight: 700, textDecoration: 'none' }}>
                  Ver todas mis ayudas →
                </Link>
              </div>
            </div>

            <p style={{ textAlign: 'center', fontSize: 12, color: '#b08060', marginTop: 20 }}>
              Resultados orientativos. Los importes y requisitos dependen de cada convocatoria y situación personal.
            </p>
          </div>
        </section>
